...
def unshard(
    ...
    shard_filename = f"{os.path.join(input_dir, base_filename)}-{num_shards - 1:05}-of-{num_shards - 1:05}.bin"
    ...
    open(f"{os.path.join(output_dir, base_filename)}.bin", "w+").close()
    full_idx_map = np.memmap(
        f"{os.path.join(output_dir, base_filename)}.bin",
        shape=(SHARD_SIZE * (num_shards - 1) + final_shard_size),
        mode="w+",
        order="C",
    )
    ...
    for i in tqdm(range(num_shards)):
        ...
        shard_filename = f"{os.path.join(input_dir, base_filename)}-{i:05}-of-{num_shards - 1:05}.bin"
        print(shard_filename)
        shard_memmap = np.memmap(shard_filename, mode="r", order="C")

        size = SHARD_SIZE if i != num_shards - 1 else final_shard_size
